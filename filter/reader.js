...
const organizeItem = (object, index, input) => {
	//if keyword existing in paragraph, we will take the paragraph and the page.
  if (object.includes(input[0])) {
    ...
    if (listSlice.length > 1) {
      listSlice = listSlice.replace(
        "edital",
        '<b style="color: red; font-size:16px;">edital</b>'
      );
      count = 1;
      while (count <= input.length) {
        listSlice = listSlice.replace(
          `${input[count]}`,
          `<b style="color: blue; font-size:16px;">${input[count]}</b>`
        );
        count++;
      }
...
const pdfReaderMain = (file, input) =>
  new Promise((resolve, reject) =>
    new pdfreader.PdfReader().parseBuffer(file, (err, item) => {
      if (err) return;
      if (!item) {
        //The last process. Here we will filter the pdf based on our keywords and return a array of objects
        Object.keys(pdfobjects).map(elm => {
          matchItem(pdfobjects, elm, input[0]);
        });
        sent.push(
          Object.keys(pdfobjects).length >= 1
            ? `Edital foi achado na página ${Object.keys(pdfobjects)}`
            : "Edital não foi encontrado."
        );
        Object.keys(pdfobjects).map(elm => {
      ...
      if (item.page) {
        //Organize rows will "organize" the text
